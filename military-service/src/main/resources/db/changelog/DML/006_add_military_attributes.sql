WITH attributes AS (SELECT d.id    AS attribute_id,
                    ...
                    FROM military_service.military_attribute_definitions d
                             JOIN military_service.ranks r ON r.id = d.rank_id
                    WHERE d.name LIKE 'Дата получения звания "%"'),
     rank_militaries AS (SELECT m.id         AS military_id,
                                r.level      AS rank_level,
                                m.birth_date AS birth_date
                         FROM military_service.militaries m
                                  JOIN military_service.ranks r ON r.id = m.rank_id
                         WHERE m.rank_id IS NOT NULL)
INSERT
INTO military_service.military_attributes (military_id, attribute_definition_id, value)
